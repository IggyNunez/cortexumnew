...
export const generateBotResponse = async (sessionId: string, userMessage: string): Promise<ChatMessage> => {
  ...
  if (lowercaseMessage.includes('pricing') || lowercaseMessage.includes('cost')) {
    ...
    botResponseText = "I love sharing our success stories! 💫 One of our clients, Bommorito Performance, transformed their elite training center with an AI-rebuilt website and social media integration that automatically identifies and nurtures athlete prospects, resulting in 185% more qualified leads! Another client, SoftRx, increased social engagement by 217% using our AI content optimization tools. Would you like to hear how we might achieve similar results for your specific industry?";
  } else if (lowercaseMessage.includes('athlete') || lowercaseMessage.includes('sport') || lowercaseMessage.includes('recruit')) {
    botResponseText = "Great question about our sports marketing solutions! 🏆 Our work with Bommorito Performance showcases how we revolutionize athlete recruitment. We built an AI-powered system that identifies potential athletes through social media activity, analyzes their performance metrics, and creates personalized outreach campaigns. Their lead pipeline increased by 185% and conversion rates improved by 67%. Would you like to learn how we could implement similar solutions for your sports-focused business?";
  } else if (lowercaseMessage.includes('lead') || lowercaseMessage.includes('prospect') || lowercaseMessage.includes('generation')) {
    botResponseText = "Lead generation is where our AI really shines! ✨ For Bommorito Performance, we developed an intelligent social media scraper that identifies potential athletes based on specific metrics and behavioral patterns. The system then nurtures these prospects with personalized content journeys, resulting in 3.4x digital engagement growth. Our AI solutions can scan thousands of profiles to find your ideal prospects while you focus on building relationships. How are you currently handling lead generation?";
  }
